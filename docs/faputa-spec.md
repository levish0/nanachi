# .faputa DSL Specification (Draft)

faputa의 자체 문법 기술 언어. winnow 기반 Rust 파서 코드를 생성하기 위한 DSL.

## 설계 원칙

- **자체 문법**: Rust도 pest도 winnow도 아닌 faputa 전용 DSL
...
```
.faputa 파일
  → faputa_meta: winnow로 .faputa 파싱 → AST
  → faputa_meta optimizer: prefix factorization, literal 병합, alt→dispatch 변환 등
  → OptimizedAST
  → faputa_generator: Rust + winnow 코드 emit
